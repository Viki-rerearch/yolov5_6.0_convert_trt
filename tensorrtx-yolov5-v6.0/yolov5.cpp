...
//extern int Yolo::INPUT_W;
//extern int Yolo::INPUT_H;
//extern int Yolo::CLASS_NUM;
...
ICudaEngine* build_engine_p6(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name) {
    ...
    /* ------ detect ------ */
    IConvolutionLayer* det0 = network->addConvolutionNd(*c3_23->getOutput(0), 3 * (CLASS_NUM + 5), DimsHW{ 1, 1 }, weightMap["model.33.m.0.weight"], weightMap["model.33.m.0.bias"]);
    IConvolutionLayer* det1 = network->addConvolutionNd(*c3_26->getOutput(0), 3 * (CLASS_NUM + 5), DimsHW{ 1, 1 }, weightMap["model.33.m.1.weight"], weightMap["model.33.m.1.bias"]);
    IConvolutionLayer* det2 = network->addConvolutionNd(*c3_29->getOutput(0), 3 * (CLASS_NUM + 5), DimsHW{ 1, 1 }, weightMap["model.33.m.2.weight"], weightMap["model.33.m.2.bias"]);
    IConvolutionLayer* det3 = network->addConvolutionNd(*c3_32->getOutput(0), 3 * (CLASS_NUM + 5), DimsHW{ 1, 1 }, weightMap["model.33.m.3.weight"], weightMap["model.33.m.3.bias"]);
...
bool parse_args(int argc, char** argv, std::string& wts, std::string& engine, bool& is_p6, float& gd, float& gw, std::string& img_dir) {
    ...
    if (std::string(argv[1]) == "-s") {    //模型转换
        wts = std::string(argv[2]);
        engine = std::string(argv[3]);
        if (std::string(argv[5]) == "-whc"){
            //INPUT_W = (int)argv[6];
            //INPUT_H = (int)argv[7];
            //CLASS_NUM = (int)argv[8];
            INPUT_W = atoi(argv[6]);
            INPUT_H = atoi(argv[7]);
            CLASS_NUM = atoi(argv[8]);
            std::cerr << "输入图像尺寸为" << INPUT_W << std::endl;
        }
        if (argc == 11) {
            DEVICE = atoi(argv[10]);
        }
    ...
    else if (std::string(argv[1]) == "-d") {   //推理
        engine = std::string(argv[2]);
        img_dir = std::string(argv[3]);
        INPUT_W = atoi(argv[5]);
        INPUT_H = atoi(argv[6]);
        CLASS_NUM = atoi(argv[7]);
        CONF_THRESH = atoi(argv[9]);
        DEVICE = atoi(argv[11]);
        std::cerr << "DEVICE:" << DEVICE << std::endl;
    } else {
        return false;
...
int main(int argc, char** argv) {
    ...
    //INPUT_H = 640;//添加宽高标签数参数
    //INPUT_W = 640;
    //CLASS_NUM = 80;

    //int argc = 4;
    //char* argv[] = { NULL, "-d", "E:/fitow_project/YOLOV5/desktop_tensorrtx_yolov5/tensorrtx-yolov5-v6.0/build/Debug/yolov5l.engine","E:/fitow_project/YOLOV5/desktop_tensorrtx_yolov5/tensorrtx-yolov5-v6.0/build/Debug/images"};
    if (!parse_args(argc, argv, wts_name, engine_name, is_p6, gd, gw, img_dir)) {
        std::cerr << "arguments not right!" << std::endl;
        std::cerr << "./yolov5 -s [.wts] [.engine] [n/s/m/l/x/n6/s6/m6/l6/x6 or c/c6 gd gw] -whc train_size_wide train_size_height class_num -dev device_id // serialize model to plan file" << std::endl;
        std::cerr << "./yolov5 -d [.engine] ../samples -whc train_size_wide train_size_height class_num -thresh CONF_THRESH -dev device_id  // deserialize plan file and run inference" << std::endl;
        return -1;
    }
    cudaSetDevice(DEVICE);  //指定显卡
